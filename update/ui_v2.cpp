...
UI::UI(){
	expDatabase.loadVctr("source-file-credit.csv");			            //change filename to load in a CreditExpenditure
	userCmd=0;
}
...
int UI::launchUI(){
	...
	float tmpValue, tmpCash;				          //added temp variable for storing cash payment
	...
	while(infLoop==0){
		...
		switch(userCmd){
			...
			case 0:   //exit program
				expDatabase.exportVctr("source-file-credit.csv");     //change file to save database
				break;
			...
			case 1:	  //view all entries in database
				...
				cout << setfill(' ') << setw(7) << "date"
					 ...
					 << setfill(' ') << setw(25) << "cash Payment"
					 << setfill(' ') << setw(20) << "credit Balance\n"; 
			...
			case 3:   //add entry
				...
				cout << "please enter the cash payment for the expense =>";
				tmpUsrIn = getUsrInput(price,0);
				tmpCash =  atof(tmpUsrIn.c_str());                           //additional input parameter for user
				
				expDatabase.addEntry(tmpCurrency,tmpValue,tmpDescription,tmpCash); 
